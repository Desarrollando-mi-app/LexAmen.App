...
const MODE_COLORS: Record<PomodoroMode, string> = {
  work: "var(--gz-gold)",
  shortBreak: "var(--gz-sage)",
  longBreak: "var(--gz-navy)",
};
...
export function PomodoroTimer({ variant = "card" }: PomodoroTimerProps) {
    ...
    if (mode === "work") {
      ...
      if (typeof window !== "undefined" && "Notification" in window && Notification.permission === "granted") {
        new Notification("Pomodoro completado", {
          body: `Sesion ${newCount} terminada. Tiempo de ${nextMode === "longBreak" ? "descanso largo" : "descanso corto"}.`,
        });
      }
    } else {
      // Break finished
      ...
      if (typeof window !== "undefined" && "Notification" in window && Notification.permission === "granted") {
        new Notification("Descanso terminado", {
          body: "Vamos con otra sesion de trabajo.",
        });
      }
    }
  ...
  useEffect(() => {
    ...
    if (isRunning) {
      ...
      document.title = `[${mins}:${secs}] ${MODE_LABELS[mode]} — Studio Iuris`;
    } else {
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <p className="font-cormorant text-[16px] !font-bold text-gz-ink">Pomodoro</p>
              {pomodoroCount > 0 && (
          ...
          <svg className="h-5 w-5 text-gz-ink-light" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
  ...
  const wrapperClass =
    variant === "card"
      ? "rounded-[4px] border border-gz-rule bg-white p-5"
      : "";
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="rounded-[3px] p-1.5 text-gz-ink-light hover:text-gz-ink hover:bg-gz-cream-dark/50 transition-colors"
            title="Configuracion"
          >
        ...
        <>
          ...
          <div className="flex justify-center">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span
                  className="font-ibm-mono text-[36px] font-bold"
                  style={{ color: MODE_COLORS[mode] }}
                >
                  {mins}:{secs}
                </span>
                <span className="mt-1 font-ibm-mono text-[10px] uppercase tracking-[2px] text-gz-ink-light">
                  {MODE_LABELS[mode]}
                </span>
              </div>
            </div>
          ...
          <div className="mt-4">
            <input
              ...
              placeholder="Etiquetar sesion..."
              className="w-full rounded-[3px] border border-gz-rule px-3 py-1.5 font-archivo text-[13px] text-gz-ink placeholder:text-gz-ink-light/50 focus:border-gz-gold focus:outline-none"
          ...
          <div className="mt-4 flex items-center justify-center gap-3">
            ...
            <button
              ...
              className="rounded-[3px] px-6 py-2 font-archivo text-[13px] font-semibold text-white transition-colors"
...
function SettingsPanel({
  ...
  return (
    <div className="space-y-3">
      <div>
        <label className="block font-ibm-mono text-[10px] uppercase tracking-[1px] text-gz-ink-light font-medium mb-1">
          Trabajo (min)
        </label>
        <input
          type="number"
          ...
          className="w-full rounded-[3px] border border-gz-rule px-3 py-2 font-ibm-mono text-[13px] text-gz-ink text-center focus:border-gz-gold focus:outline-none"
          style={{ backgroundColor: "var(--gz-cream)" }}
        />
      </div>
      <div>
      ...
      <div className="flex items-center gap-2 pt-1">
        <button
          onClick={onCancel}
          className="flex-1 rounded-[3px] border border-gz-rule px-3 py-2 font-archivo text-[13px] font-medium text-gz-ink-mid hover:bg-gz-cream-dark/50 transition-colors"
        >
          Cancelar
        </button>
        <button
          onClick={() => onSave({ work, shortBreak, longBreak })}
          className="flex-1 rounded-[3px] bg-gz-gold px-3 py-2 font-archivo text-[13px] font-semibold text-white hover:bg-gz-gold/90 transition-colors"
        >
          Guardar
        </button>
      </div>
