...
/**
 * PATCH /api/admin/timeline/[id] — update a timeline item
 */
export async function PATCH(
  ...
  try {
    ...
    if (body.eventos !== undefined) {
      // Validate eventos JSON
      try {
        const parsed = JSON.parse(body.eventos as string);
      ...
      data.eventos = body.eventos as string;
    }
    if (body.escala !== undefined) data.escala = (body.escala as string) || "dias";
    if (body.rangoMin !== undefined) data.rangoMin = body.rangoMin as number;
    if (body.rangoMax !== undefined) data.rangoMax = body.rangoMax as number;
...
/**
 * DELETE /api/admin/timeline/[id] — delete a timeline item
 */
